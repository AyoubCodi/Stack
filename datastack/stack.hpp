...
{
	long double x, y; // 2 long doubles
...
class StackManager // Definition of the class
{
	...
	public:
		...
		Stack add(Stack st, double a, double b)
		{
			...
			element->next = st;
			printf("Adresses of the struct elements : %p, %p, %p\n", &element->x, &element->y, &element->next);
		...
		void print(Stack st)
		{
			...
			while(!is_empty(st))
			{
				printf("Stack values %Lf, %Lf\nAdresses of elements : %p, %p\n", st->x, st->y, &st->x, &st->y);
				st = st->next;
			}
		}
